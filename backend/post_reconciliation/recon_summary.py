...
def build_reconciliation_summary(
    *,
    ...
    total_by_showroom = (
        delivery_recon
        .groupby("showroom_del")
        .size()
    )

    matched_del = delivery_recon[
    ...
    mismatch_del = delivery_recon[
        delivery_recon["status"] == "Not Matched"
    ]
    mismatch_by_showroom = (
        mismatch_del
        .groupby("showroom_del")
        .size()
    )
    ...
    delivery_summary_rows.append(
        ("Total Deliveries reported to us", "", total_deliveries)
    )
    ...
    # ---- Matched ----
    delivery_summary_rows.append(
        ("Matched with RTO", "", matched_del.shape[0])
    )
    ...
    # ---- Mismatch ----
    delivery_summary_rows.append(
        ("Mismatch with RTO", "", mismatch_del.shape[0])
    )
    ...
    prev_delivery = rto_recon[
        rto_recon["status"] == "Matched (Previous)"
    ]

    missing_delivery = total_rto - (matched_rto.shape[0] + 
                                    non_up32.shape[0]+
                                    prev_delivery.shape[0])

    rto_summary_rows = [
        ("Total RTO records reported to us", "", total_rto),
        ("Matched with Delivery Records", "", matched_rto.shape[0]),
    ]

    for showroom, count in matched_rto_by_showroom.items():
        rto_summary_rows.append(
            (f"{showroom}", count, "")
        )
    ...
    rto_summary_rows.extend([
        ...
        (
            "Delivered Before Month",
            "",
            prev_delivery.shape[0],
        ),
        (
            "Final difference (Need clarification)",
            "",
            missing_delivery,
        ),
    ])
